...
def main() -> None:
    ...
    cur = conn.cursor()
    cur.execute("SELECT table_name, table_comment FROM information_schema.tables WHERE table_schema=%s", (d["name"],))
    tcomment = dict(cur.fetchall())
    ...
    out = ["# 테이블 설계서 — stock_dealings", "",
           ...
           f"- DBMS: MariaDB 10.11, 문자셋 utf8mb4, 총 **{len(cols)}개 테이블**",
